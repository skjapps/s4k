...
// Images
static const unsigned char image_battery_full_bits[] U8X8_PROGMEM = { 0x00, 0x00, 0x00, 0xf0, 0xff, 0x7f, 0x08, 0x00, 0x80,
                                                                      0x68, 0xdb, 0xb6, 0x6e, 0xdb, 0xb6, 0x61, 0xdb, 0xb6, 0x61, 0xdb, 0xb6, 0x61, 0xdb, 0xb6, 0x61, 0xdb, 0xb6, 0x61, 0xdb, 0xb6, 0x6e, 0xdb,
                                                                      0xb6, 0x68, 0xdb, 0xb6, 0x08, 0x00, 0x80, 0xf0, 0xff, 0x7f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

static const unsigned char image_bluetooth_bits[] U8X8_PROGMEM = { 0x80, 0x00, 0x40, 0x01, 0x40, 0x02, 0x44, 0x04, 0x48, 0x04,
                                                                   0x50, 0x02, 0x60, 0x01, 0xc0, 0x00, 0x60, 0x01, 0x50, 0x02, 0x48, 0x04, 0x44, 0x04, 0x40, 0x02, 0x40, 0x01, 0x80, 0x00, 0x00, 0x00 };

static const unsigned char image_bluetooth_connected_bits[] U8X8_PROGMEM = { 0x80, 0x00, 0x40, 0x01, 0x40, 0x02, 0x44, 0x04,
                                                                             0x48, 0x04, 0x52, 0x12, 0x64, 0x09, 0xcc, 0x0c, 0x64, 0x09, 0x52, 0x12, 0x48, 0x04, 0x44, 0x04, 0x40, 0x02, 0x40, 0x01, 0x80, 0x00, 0x00, 0x00 };
...
static const int MOUSE_SPEED = 20;         // Default speed = 100
static const int MAX_STICK = 16384;        // Max movement of stick = 16384
static const int HOLD_TIME = 200;          // How long to hold for 2nd input
static const int DOUBLE_TAP_TIME = 100;    // How long to hold for multi input
static const int PRESS_TIME = 200;         // Press buttons in multi-button mode for 200ms
...
static const int inputPins[] = { BUTTON_LEFT_PIN, BUTTON_RIGHT_PIN, BUTTON_UP_PIN, BUTTON_DOWN_PIN, BUTTON_A_PIN };
...
int inputsPressed[] = { 0, 0, 0, 0, 0 };       // storing the instance pressed buttons for one update
int inputsDisplay[] = { 0, 0, 0, 0, 0, 0 };    // Which inputs to display (for use with Screen) (Left, Right, Up, Down, A, B)
...
void loop() {
  ...
  // Setting up/down/left/right inputs the same before controller display
  memset(inputsDisplay, inputs[0], sizeof(inputs[0] * 4));
...
void ControlSchemeSelect() {
  ...
  if (inputsPressed[4]) {
    ...
    gamepadControlScheme = (gamepadControlScheme + 1) % 2 + 1;  // Increment by 1 for left button
  } else if (inputsPressed[1]) {
    gamepadControlScheme = gamepadControlScheme % 2 + 1;  // No change for right button (keeps current value)
  }
  ...
  if (compositeHID.isConnected()) {
    ...
    switch (mode) {
      ...
      case 1:
        ...
        // Control scheme 2 = holding button 200ms = B
        if (gamepadControlScheme == 2) {
        ...
        // Move Left Stick
        gamepad->setLeftThumb((MAX_STICK + (inputs[0] * -MAX_STICK) + (inputs[1] * (MAX_STICK - 1))),
                              (MAX_STICK + (inputs[2] * -MAX_STICK) + (inputs[3] * (MAX_STICK - 1))));
        gamepad->sendGamepadReport();
